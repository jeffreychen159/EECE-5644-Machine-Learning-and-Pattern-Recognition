...
% Generate independent samples from the same GMM
xv = randGMM(N,alpha_true,mu_true,Sigma_true);
...
while ~Converged
    ...
    t = t+1; 
    displayProgress(t,x,xv,alpha,mu,Sigma);
end
%keyboard,

%%%
function displayProgress(t,x,xv,alpha,mu,Sigma)
figure(1),
if size(x,1)==2
    subplot(1,2,1), cla,
    plot(x(1,:),x(2,:),'b.'); 
...
logLikelihoodv = sum(log(evalGMM(xv,alpha,mu,Sigma)));
plot(t,logLikelihood,'b.'); hold on,
plot(t,logLikelihoodv,'m.'); hold on,
